...
2. run the commands in this order to activate the 3 auction processes/nodes in different terminals:
    go run auctionNode.go --id=node1 --port=:5001 --primary=true
    go run auctionNode.go --id=node2 --port=:5002 --primary=false
    go run auctionNode.go --id=node3 --port=:5003 --primary=false
